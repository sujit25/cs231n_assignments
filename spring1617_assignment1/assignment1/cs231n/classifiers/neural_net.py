...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        # relu activation applied over linear mapping
        ...
        loss += reg * np.sum(pow(W1, 2)) + reg * np.sum(pow(W2, 2))  # L1 & L2 regularization
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in xrange(num_iters):
            ...
            #########################################################################
            random_nums = np.random.randint(0, num_train, batch_size, dtype=int)
            X_batch = X[random_nums]
            y_batch = y[random_nums]
    ...
    def predict(self, X):
        ...
        ###########################################################################
        opt_w1, opt_b1 = self.params['W1'], self.params['b1']
        opt_w2, opt_b2 = self.params['W2'], self.params['b2']
        opt_h1_scores = np.maximum(0, np.dot(X, opt_w1) + opt_b1)   # relu activation unit - h1_scores - 300 X 100\n",
        opt_scores = np.dot(opt_h1_scores, opt_w2) + opt_b2         # relu activation unit- h1_scores- 300 X 100, W2 - 100X 3, scores - 300 X 3\n",
        y_pred = np.argmax(opt_scores, axis=1)                      # compute index of max score along row\n",
        ###########################################################################
        #                              END OF YOUR CODE                           #
        ###########################################################################
